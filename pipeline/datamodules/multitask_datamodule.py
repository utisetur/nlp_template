...
from src.text_utils import prepare_data, load_obj
...
class MultiTaskDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        # load datasets
        self.terra_train_data = pd.read_json(path_or_buf=self.cfg.datamodule.terra.data_path + 'train.jsonl',
                                             lines=True).set_index('idx')
        self.terra_val_data = pd.read_json(path_or_buf=self.cfg.datamodule.terra.data_path + 'val.jsonl',
                                           lines=True).set_index('idx')
        self.terra_test_data = pd.read_json(path_or_buf=self.cfg.datamodule.terra.data_path + 'test.jsonl',
                                            lines=True).set_index('idx')

        self.russe_train_data = pd.read_json(path_or_buf=self.cfg.datamodule.russe.data_path + 'train.jsonl',
                                             lines=True).set_index('idx')
        self.russe_val_data = pd.read_json(path_or_buf=self.cfg.datamodule.russe.data_path + 'val.jsonl',
                                           lines=True).set_index('idx')
        self.russe_test_data = pd.read_json(path_or_buf=self.cfg.datamodule.russe.data_path + 'test.jsonl',
                                            lines=True).set_index('idx')
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            ...
            self.terra_train_dataset = terra_dataset_class(
                data=self.terra_train_data, tokenizer=self.tokenizer,
            )
            self.terra_valid_dataset = terra_dataset_class(
                data=self.terra_val_data, tokenizer=self.tokenizer,
            )

            russe_dataset_class = load_obj(self.cfg.datamodule.russe.class_name)
            self.russe_train_dataset = russe_dataset_class(
                data=self.russe_train_data, tokenizer=self.tokenizer,
            )
            self.russe_valid_dataset = russe_dataset_class(
                data=self.russe_val_data, tokenizer=self.tokenizer,
            )
